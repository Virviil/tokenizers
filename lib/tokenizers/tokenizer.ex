defmodule Tokenizers.Tokenizer do
  ...
  def from_pretrained(identifier, opts \\ []) do
    ...
    additional_special_tokens = Keyword.get(opts, :additional_special_tokens, [])
    ...
    if opts[:use_cache] do
      with {:ok, response} <- request(http_client, Keyword.put(http_opts, :method, :head)) do
        ...
        if File.exists?(file_path) do
          from_file(file_path, additional_special_tokens)
        else
  ...
  """
  @doc section: :creators
  @spec from_file(path :: String.t(), additional_special_tokens :: [String.t()]) ::
          {:ok, Tokenizer.t()} | {:error, term()}
  defdelegate from_file(path, additional_special_tokens \\ []),
  ...
  """
  @doc section: :creators
  @spec from_buffer(data :: String.t(), additional_special_tokens :: [String.t()]) ::
          {:ok, Tokenizer.t()} | {:error, term()}
  defdelegate from_buffer(data, additional_special_tokens \\ []),
    to: Tokenizers.Native,
  ...
  defdelegate save(tokenizer, path, pretty \\ true), to: Tokenizers.Native, as: :tokenizer_save
  ...
  * `:direction` (default: `:right`) - the direction to use when overflowing the model's input.
  ...
  defdelegate set_truncation(tokenizer, max_length \\ 512, opts \\ []),
    to: Tokenizers.Native,
  ...
  * `:pad_token` (default: `"<pad>"`) - the token to use for padding.
  ...
  defdelegate encode(tokenizer, input, add_special_tokens \\ true),
    to: Tokenizers.Native,
    as: :tokenizer_encode
  ...
  @spec encode_batch(
          ...
          {:ok, [Encoding.t()]} | {:error, term()}
  defdelegate encode_batch(tokenizer, input, add_special_tokens \\ true),
    to: Tokenizers.Native,
  ...
  defdelegate decode(tokenizer, ids, skip_special_tokens \\ true),
    to: Tokenizers.Native,
    as: :tokenizer_decode
  ...
  @spec decode_batch(
          ...
          {:ok, [String.t()]} | {:error, term()}
  defdelegate decode_batch(tokenizer, sentences, skip_special_tokens \\ true),
    to: Tokenizers.Native,
